...
ABUNDANTS = abundant.take_while{|n| n < MAX_BOUND}

candidates = [true] * MAX_BOUND
ABUNDANTS.each do |a|
  ABUNDANTS.take_while{|b| a+b <= MAX_BOUND}.each do |b|
    candidates[a+b] = false
  end
end

puts candidates.each_with_index.reduce([]){|a, (ok, i)| ok ? (a << i) : a}.reduce(:+)
